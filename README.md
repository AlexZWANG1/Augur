...
Augur 不是一个普通 RAG app。RAG 的核心动作是“问问题 -> 检索 chunk -> 生成答案”；Augur 的核心动作是“读材料 -> 编译结构 -> 暴露矛盾 -> 更新判断 -> 回放路径”。

Augur 也不是普通 note graph。note graph 显示的是文件关系，Augur 显示的是 reasoning state：source 为什么支持 signal，哪个证据改变了 conviction，哪个 contradiction 需要人介入，哪个判断已经跨过验证窗口。

Augur 更接近一个 **reasoning substrate**：模型每次输出都不是一次性文本，而是对一个持续存在的思维图谱做增量更新。人在这个过程中不是被动读答案，而是监督模型如何组织知识。
...
Replay scene 把一次推理拆成可滑动的 scene：本步看到什么、判断什么、导向什么、如何改变下一步。它是 Augur 最核心的产品方向之一：把跨 session 的 reasoning trail 变成用户可以检查、暂停、质疑和重放的界面。
...
Augur is not a standard RAG app. RAG usually turns a question into retrieved chunks and a generated answer. Augur turns source material into persistent reasoning objects that can be inspected, contradicted, updated, and replayed.

Augur is also not a standard note graph. A note graph shows file topology. Augur shows reasoning state: why a source supports a signal, which contradiction matters, what changed conviction, and where a human should intervene.

The product direction is a **reasoning substrate**: every model run becomes an incremental update to a persistent thought graph instead of an isolated answer.
